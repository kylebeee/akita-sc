...
import { abimethod, Address, methodSelector } from '@algorandfoundation/algorand-typescript/arc4'
...
export class Poll extends classes(AkitaBaseContract, ContractWithGate) {
    ...
    private getUserImpact(user: Account): uint64 {
        // TODO: update this when social is written in puya-ts
        const impactCall = itxn
            .applicationCall({
                appId: super.getAppList().impact,
                onCompletion: OnCompleteAction.NoOp,
                appArgs: [methodSelector(AkitaSocialImpact.prototype.getUserImpact), user],
                fee: 0,
            })
            .submit()
