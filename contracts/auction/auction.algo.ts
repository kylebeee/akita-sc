...
import {
    AKITA_AUCTION_COMPOSABLE_PERCENTAGE_KEY,
    AKITA_AUCTION_SALE_PERCENTAGE_MAXIMUM_KEY,
    AKITA_AUCTION_SALE_PERCENTAGE_MINIMUM_KEY,
} from '../dao/constants'
...
export class Auction extends ContractWithOptInCreatorOnlyArc59AndGate {
        ...
        if (this.highestBid.value > 0) {
            if (this.bidAsset.value.id === 0) {
                if (this.bidsByAddress(this.txn.sender).exists) {
                    return this.highestBid.value + this.bidMinimumIncrease.value + bidMBR
                }
                return this.highestBid.value + this.bidMinimumIncrease.value + bidTotalMBR
            }
    ...
    private getBidFee(amount: uint64): uint64 {
        ...
        // const fee = this.wideRatio(amount, this.bidFee.value, DIVISOR)
    ...
    private getAmounts(amount: uint64): RoyaltyAmounts {
        ...
        // let creatorAmount = this.wideRatio(amount, this.creatorRoyalty.value, DIVISOR)
        ...
        // const akitaTaxRate = maxTax - this.wideRatio((maxTax - minTax), (impact - 1), IMPACT_DIVISOR)
    ...
    private transferPurchaseToBuyer(buyer: Address): void {
        ...
        if (buyer.isOptedInToAsset(this.prize.value)) {
            ...
            this.arc59OptInAndSend(buyer, this.prize.value, prizeAmount, true)
        }
    }
    ...
    private completeAsaPayments(amount: uint64, buySideMarketplace: Address): void {
        ...
        if (this.prize.value.creator.isOptedInToAsset(this.bidAsset.value)) {
            ...
            this.arc59OptInAndSend(this.prize.value.creator, this.bidAsset.value, amounts.creator, false)
        }
        ...
        if (this.marketplace.value.isOptedInToAsset(this.bidAsset.value)) {
            ...
            this.arc59OptInAndSend(this.marketplace.value, this.bidAsset.value, amounts.marketplace, false)
        }
        ...
        if (buySideMarketplace.isOptedInToAsset(this.bidAsset.value)) {
            ...
            this.arc59OptInAndSend(buySideMarketplace, this.bidAsset.value, amounts.marketplace, false)
        }
        ...
        if (this.seller.value.isOptedInToAsset(this.bidAsset.value)) {
            ...
            this.arc59OptInAndSend(this.seller.value, this.bidAsset.value, amounts.seller, false)
        }
    }
    ...
    @abi.readonly
    isLive(): boolean {
        return globals.round <= this.startingRound.value && globals.round >= this.endingRound.value
    }
    ...
    claimRafflePrize(): void {
        ...
        if (this.bidAsset.value.id === 0) {
        ...
        } else if (this.raffleWinner.value.isOptedInToAsset(this.bidAsset.value)) {
            sendAssetTransfer({
            ...
            this.arc59OptInAndSend(this.raffleWinner.value, this.bidAsset.value, this.raffleAmount.value, false)
        }
