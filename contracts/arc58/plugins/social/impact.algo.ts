...
import {
    AkitaAppIDsAkitaSocialPlugin,
    AkitaAppIDsStakingPlugin,
    AkitaAppIDsSubscriptions,
    AkitaCollectionsPrefixAKC,
    AkitaCollectionsPrefixAOG,
    AkitaNFTCreatorAddress,
...
export class AkitaSocialImpact extends Contract {
    ...
    private isSubscribed(address: Address, index: uint64): { active: boolean; index: uint64; streak: uint64 } {
        ...
        const lastWindowStart =
            globals.latestTimestamp - (((globals.latestTimestamp - info.startDate) % info.interval) + info.interval)
