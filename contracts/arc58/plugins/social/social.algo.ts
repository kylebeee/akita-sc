...
export class AkitaSocialPlugin extends ContractWithGate {
    ...
    private getTax(impact: uint64): uint64 {
        ...
        // const taxRate = maxTax - this.wideRatio((maxTax - minTax), (impact - 1), IMPACT_DIVISOR)
        const reactionFee = this.akitaDaoAppID.globalState(SOCIAL_REACT_KEY) as uint64
        let tax = wideRatio([reactionFee, taxRate], [10_000])
        // let tax = this.wideRatio(reactionFee, taxRate, DIVISOR)
        if (tax === 0) {
            tax = 1
        }
        return tax
    ...
    private canCallArc58OptIn(sender: Address, walletAppID: AppID): boolean {
        return sendMethodCall<typeof AbstractedAccount.prototype.arc58_canCall, boolean>({
            ...
            methodArgs: [AppID.fromUint64(AkitaAppIDsOptinPlugin), sender],
            fee: 0,
        })
    }
    ...
    private arc59OptInAndSendReactionPayments(
        ...
        >({
            ...
            methodArgs: [recipientAppID.address, akitaToken.id],
            fee: 0,
        })
    ...
    private updateStreak(address: Address): void {
        ...
        const thisWindowStart = globals.latestTimestamp - ((globals.latestTimestamp - meta.startDate) % ONE_DAY)
    ...
    private createPost(sender: AppID, rekeyBack: boolean, cid: bytes<36>, gateID: uint64, isAmendment: boolean): void {
    ...
    editPost(sender: AppID, rekeyBack: boolean, cid: bytes<36>, gateID: uint64, amendment: bytes32): void {
    ...
    replyPost(sender: AppID, rekeyBack: boolean, cid: bytes<36>, ref: bytes32, gateID: uint64, args: bytes[]): void {
        this.createReply(sender, rekeyBack, cid, ref, gateID, args, false)
    }

    replyAsset(sender: AppID, rekeyBack: boolean, cid: bytes<36>, ref: AssetID, gateID: uint64): void {
    ...
    replyAddress(sender: AppID, rekeyBack: boolean, cid: bytes<36>, ref: Address, gateID: uint64, args: bytes[]): void {
    ...
    replyApp(sender: AppID, rekeyBack: boolean, cid: bytes<36>, ref: AppID, gateID: uint64): void {
    ...
    initMeta(
        ...
        if (automated) {
            ...
            sendMethodCall<typeof AkitaSocialImpact.prototype.cacheMeta, uint64>({
                sender: sender.address,
                applicationID: AppID.fromUint64(AkitaAppIDsAkitaSocialImpactPlugin),
                methodArgs: [0, AppID.fromUint64(0), AssetID.fromUint64(0)],
                fee: 0,
            })
        ...
        const impact = sendMethodCall<typeof AkitaSocialImpact.prototype.cacheMeta, uint64>({
            sender: sender.address,
            applicationID: AppID.fromUint64(AkitaAppIDsAkitaSocialImpactPlugin),
            methodArgs: [subscriptionIndex, NFD, akitaNFT],
            fee: 0,
        })
