...
export class FollowerIndexGate extends AkitaBaseContract {
  ...
  private followerIndexGate(user: Address, index: uint64, follower: Address, op: Operator, value: uint64): boolean {
    ...
    const isFollowerTxn = itxn
      .applicationCall({
        ...
        appArgs: [
          methodSelector(AkitaSocialPlugin.prototype.isFollower),
          user,
          index,
          follower
        ]
  ...
  check(args: bytes): boolean {
    ...
    return this.followerIndexGate(info.user, params.index.native, params.follower, info.op.native, info.value.native);
  }
}