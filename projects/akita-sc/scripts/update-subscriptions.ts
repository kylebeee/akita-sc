 ...
 * 
 * This script updates the Subscriptions smart contract to a new version.
 * It follows the same patterns as deploy-universe.ts and the DAO upgrade tests.
 * 
 ...
 * 2. Apply the upgrade to the Subscriptions app via updateApp
 * 
 * Usage:
 *   ts-node scripts/update-subscriptions.ts --network testnet --mnemonic "your mnemonic" --version "1.0.1"
 */
...
function parseArgs(): UpdateOptions {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--network' || args[i] === '-n') {
            ...
            console.log(`
Usage: ts-node scripts/update-subscriptions.ts [options]
...
Examples:
  ts-node scripts/update-subscriptions.ts
  ts-node scripts/update-subscriptions.ts --network testnet --mnemonic "your mnemonic phrase here" --version "1.0.1"
  ts-node scripts/update-subscriptions.ts -n mainnet -m "your mnemonic phrase here" -v "1.1.0"
  ts-node scripts/update-subscriptions.ts --dry-run --network testnet
`)
            process.exit(0)
        }
    }
...
async function update() {
    ...
    try {
        ...
        console.log(`   Subscriptions: ${appIds.subscriptions}`)
        ...
        if (options.mnemonic || options.network === 'localnet') {
            ...
            if (balance < minBalance) {
                console.error(`❌ Insufficient balance. Need at least 50 ALGO for update operations.`)
                process.exit(1)
            }
        }
        ...
        // Compile the new Subscriptions contract
        console.log('📦 Compiling new Subscriptions contract...')
        const subscriptionsFactory = new SubscriptionsFactory({
            algorand,
            defaultSender: sender,
            defaultSigner: signer,
        })
        const compiled = await subscriptionsFactory.appFactory.compile()
        ...
        if (options.dryRun && !options.mnemonic && options.network !== 'localnet') {
            ...
            console.log(`   Target app: ${appIds.subscriptions}`)
        ...
        console.log('🔨 Building update execution...')
        ...
        const subscriptionsClient = subscriptionsFactory.getAppClientById({
            appId: appIds.subscriptions,
        })
        const newVersion = await subscriptionsClient.state.global.version()
        console.log(`   New contract version: ${newVersion}\n`)

        // Display summary
        console.log('='.repeat(80))
        console.log('✅ SUBSCRIPTIONS UPDATE COMPLETE!')
        console.log('='.repeat(80))
        console.log(`
Summary:
  Network: ${options.network}
  Contract: Subscriptions
  App ID: ${appIds.subscriptions}
