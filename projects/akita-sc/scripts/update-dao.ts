 ...
 * 
 * This script updates the AkitaDAO smart contract to a new version.
 ...
 * 2. Apply the upgrade to the DAO app via updateApp
 * 
 * Usage:
 *   ts-node scripts/update-dao.ts --network testnet --mnemonic "your mnemonic" --version "1.0.1"
 */
...
function parseArgs(): UpdateOptions {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--network' || args[i] === '-n') {
            ...
            console.log(`
Usage: ts-node scripts/update-dao.ts [options]
...
Examples:
  ts-node scripts/update-dao.ts
  ts-node scripts/update-dao.ts --network testnet --mnemonic "your mnemonic phrase here" --version "1.0.1"
  ts-node scripts/update-dao.ts -n mainnet -m "your mnemonic phrase here" -v "1.1.0"
  ts-node scripts/update-dao.ts --dry-run --network testnet
`)
            process.exit(0)
        }
    }
...
async function update() {
    ...
    try {
        ...
        // Compile the new DAO contract
        console.log('📦 Compiling new AkitaDAO contract...')
        const daoFactory = new AkitaDaoFactory({
            algorand,
            defaultSender: sender,
            defaultSigner: signer,
        })
        const compiled = await daoFactory.appFactory.compile()
        ...
        if (options.dryRun && !options.mnemonic && options.network !== 'localnet') {
            ...
            console.log(`   Target app: ${appIds.dao}`)
        ...
        const daoClient = daoFactory.getAppClientById({
            appId: appIds.dao,
        })
        const newVersion = await daoClient.state.global.version()
        console.log(`   New contract version: ${newVersion}\n`)

        // Display summary
        console.log('='.repeat(80))
        console.log('✅ AKITA DAO UPDATE COMPLETE!')
        console.log('='.repeat(80))
        console.log(`
Summary:
