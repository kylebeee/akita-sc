 ...
 * 
 * This script deploys a new version of the SubscriptionsPlugin contract.
 * 
 * Note: The SubscriptionsPlugin is not tracked in the DAO's PAL and is not
 * installed on the DAO wallet by default. This script only deploys the contract.
 * 
 * Usage:
 *   ts-node scripts/deploy-subscriptions-plugin.ts --network testnet --mnemonic "your mnemonic"
 */

import { AlgorandClient } from '@algorandfoundation/algokit-utils'
import { algorandFixture } from '@algorandfoundation/algokit-utils/testing'
import { AlgorandFixture } from '@algorandfoundation/algokit-utils/types/testing'
import { getNetworkAppIds, type AkitaNetwork } from 'akita-sdk'
...
function parseArgs(): DeployOptions {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--network' || args[i] === '-n') {
            ...
            console.log(`
Usage: ts-node scripts/deploy-subscriptions-plugin.ts [options]
...
Options:
  ...
  --version, -v <version>     Version string for the plugin (default: "0.0.2")
  --dry-run                   Show what would be deployed but don't execute
  --help, -h                  Show this help message

Examples:
  ts-node scripts/deploy-subscriptions-plugin.ts
  ts-node scripts/deploy-subscriptions-plugin.ts --network testnet --mnemonic "your mnemonic phrase here"
  ts-node scripts/deploy-subscriptions-plugin.ts -n testnet -m "your mnemonic" --version "0.0.3"
  ts-node scripts/deploy-subscriptions-plugin.ts --dry-run --network testnet
`)
            process.exit(0)
        }
    }
...
// Deploy the SubscriptionsPlugin
async function deploySubscriptionsPlugin(
    algorand: AlgorandClient,
    sender: string,
    signer: algosdk.TransactionSigner,
    daoAppId: bigint,
    version: string
): Promise<SubscriptionsPluginSDK> {
...
async function deploy() {
    ...
    try {
        ...
        if (options.dryRun) {
            ...
            console.log(`   1. Deploy new SubscriptionsPlugin contract (version: ${options.version})`)
            console.log('\n✅ Dry run complete! Run without --dry-run to execute.\n')
            process.exit(0)
        }

        // Deploy new subscriptions plugin
        console.log('📦 Deploying new SubscriptionsPlugin...')
        const newSubscriptionsPlugin = await deploySubscriptionsPlugin(
            algorand,
            sender,
            signer,
            appIds.dao,
        ...
        console.log(`   ✅ New plugin deployed: ${newSubscriptionsPlugin.appId}\n`)
...
Summary:
  ...
  New Plugin App ID: ${newSubscriptionsPlugin.appId}
  Version: ${options.version}

⚠️  IMPORTANT: Update the SDK networks.ts file with the new plugin app ID:
  subscriptionsPlugin: ${newSubscriptionsPlugin.appId}n,
`)
