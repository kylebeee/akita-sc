 ...
 *
 * This script updates the AuctionFactory smart contract to a new version.
 *
 ...
 * 2. Apply the upgrade to the AuctionFactory app via updateApp
 *
 * Usage:
 *   ts-node scripts/update-auction-factory.ts --network testnet --mnemonic "your mnemonic" --version "1.0.1"
 */
...
function parseArgs(): UpdateOptions {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--network' || args[i] === '-n') {
            ...
            console.log(`
Usage: ts-node scripts/update-auction-factory.ts [options]
...
Examples:
  ts-node scripts/update-auction-factory.ts
  ts-node scripts/update-auction-factory.ts --network testnet --mnemonic "your mnemonic phrase here" --version "1.0.1"
  ts-node scripts/update-auction-factory.ts -n mainnet -m "your mnemonic phrase here" -v "1.1.0"
  ts-node scripts/update-auction-factory.ts --dry-run --network testnet
`)
            process.exit(0)
        }
    }
...
async function update() {
    ...
    try {
        ...
        console.log(`   Auction Factory: ${appIds.auctionFactory}`)
        ...
        // Compile the new AuctionFactory contract
        console.log('📦 Compiling new AuctionFactory contract...')
        const auctionFactoryFactory = new AuctionFactoryFactory({
            algorand,
            defaultSender: sender,
            defaultSigner: signer,
        })
        const compiled = await auctionFactoryFactory.appFactory.compile()
        ...
        if (options.dryRun && !options.mnemonic && options.network !== 'localnet') {
            ...
            console.log(`   Target app: ${appIds.auctionFactory}`)
        ...
        const factoryClient = auctionFactoryFactory.getAppClientById({
            appId: appIds.auctionFactory,
        })
        const newVersion = await factoryClient.state.global.version()
        console.log(`   New contract version: ${newVersion}\n`)

        // Display summary
        console.log('='.repeat(80))
        console.log('✅ AUCTION FACTORY UPDATE COMPLETE!')
        console.log('='.repeat(80))
        console.log(`
Summary:
  Network: ${options.network}
  Contract: AuctionFactory
  App ID: ${appIds.auctionFactory}
