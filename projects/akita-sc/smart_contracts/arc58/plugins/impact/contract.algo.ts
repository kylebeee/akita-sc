...
import { AkitaSocialPlugin } from '../social/contract.algo'
...
import { Application, assert, assertMatch, Asset, BoxMap, bytes, Bytes, gtxn, op } from '@algorandfoundation/algorand-typescript'
...
export class AkitaSocialImpact extends classes(Plugin, AkitaSubBaseContract) {

    /** A map of the meta data for each user */
    meta = BoxMap<Account, arc4MetaValue>({ keyPrefix: 'm' })
    /** A map of how each akita subscription affects impact calculation */
    subscriptionStateModifier = BoxMap<uint64, uint64>({ keyPrefix: 's' })
    ...
    private isSubscribed(account: Account, index: uint64): { active: boolean; serviceID: uint64; streak: uint64 } {
        ...
        const lastWindowStart = Global.latestTimestamp - (((Global.latestTimestamp - info.startDate) % info.interval) + info.interval)
    ...
    private userImpact(account: Account, includeSocial: boolean): uint64 {
        ...
        const socialImpact = includeSocial ? this.getSocialImpactScore(account) : 0 // Social Activity | up to 250
    ...
    private getStakingImpactScore(account: Account): uint64 {
        ...
        const elapsed = Global.latestTimestamp - info.lastUpdate
    ...
    private getSocialImpactScore(account: Account): uint64 {
        return abiCall(
            ...
            {
                appId: super.getPluginAppList().social,
    ...
    private calcNFDImpactScore(NFDApp: Application): uint64 {
        ...
        if (parentAppIDBytesExist && btoi(parentAppIDBytes) === super.getAkitaAppList().akitaNFD) {
            nfdImpact += 50
        }
