...
import { SubscriptionsClient, SubscriptionsFactory } from '../artifacts/subscriptions/SubscriptionsClient'
...
describe('Subscription Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const minter = new SubscriptionsFactory({
    ...
    const results = await minter.send.create.create({ args: { version: '0.0.1', akitaDao: 0n, escrow: 0n } })
    client = results.appClient

    client.appClient.fundAppAccount({ amount: (100_000).microAlgos() })

    console.log('Subscription Address:', client.appAddress.toString())
