...
class SocialSDK {
     ...
     */
    async checkTipMbrRequirements({ sender, signer, creator, wallet = 0n, }) {
        const sendParams = this.getSendParams({ sender, signer });
     ...
     */
    async getVote({ sender, signer, ref }) {
        const sendParams = this.getSendParams({ sender, signer });
     ...
     */
    async isFollower({ sender, signer, user, index, follower }) {
        const sendParams = this.getSendParams({ sender, signer });
     ...
     */
    async initMeta({ sender, signer, user, automated = false, subscriptionIndex = 0n, nfd = 0n, akitaNFT = 0n, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async updateMeta({ sender, signer, followGateId = 0n, addressGateId = 0n, subscriptionIndex = 0n, nfd = 0n, akitaNFT = 0n, defaultPayWallId = 0n, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async post({ sender, signer, timestamp: providedTimestamp, nonce: providedNonce, cid, gateId = 0n, usePayWall = false, payWallId = 0n, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async editPost({ sender, signer, cid, amendment, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async reply({ sender, signer, timestamp: providedTimestamp, nonce: providedNonce, cid, ref, refType, gateId = 0n, usePayWall = false, payWallId = 0n, gateTxn, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
        // Fetch fees, assets, and blockchain timestamp in parallel
        const [socialFees, akitaAssets, blockchainTimestamp] = await Promise.all([
            this.getSocialFees(),
     ...
     * - Tip: reactFee in AKTA from DAO social fees
     */
    async vote({ sender, signer, ref, refType, isUp, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
        // Fetch fees and assets from DAO
        const [socialFees, akitaAssets] = await Promise.all([
            this.getSocialFees(),
     ...
     */
    async invertVote({ sender, signer, ref, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
        // Fetch fees and assets from DAO
        const [socialFees, akitaAssets] = await Promise.all([
            this.getSocialFees(),
     ...
     */
    async deleteVote({ sender, signer, ref, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async react({ sender, signer, ref, refType, nft, gateTxn, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
        // Fetch fees and assets from DAO
        const [socialFees, akitaAssets] = await Promise.all([
            this.getSocialFees(),
     ...
     */
    async deleteReaction({ sender, signer, ref, nft, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
        const group = this.socialClient.newGroup();
     ...
     */
    async follow({ sender, signer, address, gateTxn, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     * Unfollow a user
     */
    async unfollow({ sender, signer, address, index, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async block({ sender, signer, address, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     * Unblock a user
     */
    async unblock({ sender, signer, address, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async createPayWall({ sender, signer, userPayInfo, agentPayInfo, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async addModerator({ sender, signer, address, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async removeModerator({ sender, signer, address, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async ban({ sender, signer, address, expiration, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async unban({ sender, signer, address, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async flagPost({ sender, signer, ref, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async unflagPost({ sender, signer, ref, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async addAction({ sender, signer, actionAppId, content, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
     ...
     */
    async removeAction({ sender, signer, actionAppId, }) {
        const sendParams = this.getRequiredSendParams({ sender, signer });
