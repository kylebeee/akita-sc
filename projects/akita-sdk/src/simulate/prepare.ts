...
import { SimulateBuilder, SimulateOptions } from "./builder";
...
async function getGroupExecutionInfo(
  ...
  const simulateResponse = (result as any).simulateResponse ?? result;
    ...
    txns: groupResponse.txnResults.map((txn: any, i: number) => {
      ...
      if (sendParams.coverAppCallInnerTransactionFees) {
        ...
        if (originalTxn.type === algosdk.TransactionType.appl) {
          const calculateInnerFeeDelta = (itxns: any[], acc = 0n): bigint => {
...
export async function prepareGroupWithCost(
  ...
  if (sendParams.populateAppCallResources) {
    const populateGroupResource = (txns: algosdk.TransactionWithSigner[], reference: any, type: 'account' | 'app' | 'asset' | 'box') => {
      ...
      if (txnIndex >= 0) {
        const tx: any = txns[txnIndex].txn;
        if (type === 'account') {
    ...
    executionInfo.groupUnnamedResourcesAccessed?.accounts?.forEach((a: string) => populateGroupResource(group, a, 'account'));
    executionInfo.groupUnnamedResourcesAccessed?.assets?.forEach((a: number | bigint) => populateGroupResource(group, a, 'asset'));
    executionInfo.groupUnnamedResourcesAccessed?.apps?.forEach((a: number | bigint) => populateGroupResource(group, a, 'app'));
    executionInfo.groupUnnamedResourcesAccessed?.boxes?.forEach((b: algosdk.modelsv2.BoxReference) => populateGroupResource(group, b, 'box'));
  }
 ...
 */
function calculateExpectedCostFromGroup(
  group: algosdk.TransactionWithSigner[],
  simulateOptions?: SimulateOptions
): ExpectedCost {
  const hints = new Map<bigint, any>();
