...
} from '../clients/AbstractedAccountFactoryClient'
...
describe('ARC58 Plugin Permissions', () => {
    ...
    async function callPlugin(
        ...
        const optInGroup = (
            await pluginClient.createTransaction.optInToAsset({
                sender: caller.addr,
    ...
    beforeAll(async () => {
        ...
        await algorand.account.ensureFunded(aliceEOA.addr, dispenser, (100).algos())

        await algorand.account.ensureFunded(caller.addr, dispenser, (100).algos())
    ...
    test('malformed methodOffsets', async () => {
        await abstractedAccountClient.send.arc58AddPlugin({
            ...
            args: {
                ...
                lastValidRound: MAX_UINT64,
                adminPrivileges: false,
                methods: [[new Uint8Array(Buffer.from('dddd')), 0]],
            },
        })
